...
import { IdleProgressionSimulator } from './systems/IdleProgressionSimulator';
...
class GameApp {
  ...
  public async bootstrap(): Promise<void> {
    ...
    gameLoop.addCallback((dt, now) => {
      ...
      const simulatedEconomy = IdleProgressionSimulator.simulateEconomy(state, dt, {
        startAtMs: now - dt * 1000,
        efficiencyMultiplier: 1,
        maxStepSeconds: 0.5,
      });

      if (simulatedEconomy.powerGained > 0 || simulatedEconomy.goldGained > 0) {
        store.set((draft) => {
          draft.power += simulatedEconomy.powerGained;
          draft.gold += simulatedEconomy.goldGained;
          draft.stats.lifetimePower += simulatedEconomy.powerGained;
          draft.stats.lifetimeGold += simulatedEconomy.goldGained;

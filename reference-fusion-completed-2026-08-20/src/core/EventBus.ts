...
export type GameEventMap = {
  ...
  'party:character_xp_gained': { slotId: 'char_1' | 'char_2'; xpGained: number; level: number; xp: number; xpToNextLevel: number };
  'party:character_level_up': { slotId: 'char_1' | 'char_2'; oldLevel: number; newLevel: number };

  // Real-time Combat Events
  'combat:enemy_spawned': { enemy: any; stageId: string; encounterIndex: number };
  'combat:phase_changed': { phase: 'run' | 'combat' | 'reward' | 'boss_failed' | 'hero_defeated'; stageId: string; encounterIndex: number };
  ...
  'combat:enemy_attack': { enemyId: string; damage: number; remainingHeroHp: number; heroMaxHp: number };
  'combat:hero_defeated': { enemyId: string; stageId: string; isBoss: boolean };
  ...
  'combat:loot_dropped': { item: any; stageId: string; enemyId: string };

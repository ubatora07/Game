...
export class EnemyRenderer {
  ...
  public static render(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    ...
    if (def.id.includes('slime')) {
    ...
    } else if (def.id.includes('goblin') || def.id.includes('orc')) {
      // Goblin / Orc Character with ears, weapon and armor
      const isBoss = def.isBoss;
      const bScale = isBoss ? 1.4 : 1.0;
      
      // Body / Tunic
      ctx.fillStyle = def.color;
      ctx.fillRect(-16 * bScale, -10 * bScale, 32 * bScale, 36 * bScale);
      
      // Head
      ctx.fillRect(-14 * bScale, -30 * bScale, 28 * bScale, 22 * bScale);
      
      // Pointed Ears
      ctx.beginPath();
      ctx.moveTo(-14 * bScale, -24 * bScale);
      ctx.lineTo(-24 * bScale, -28 * bScale);
      ctx.lineTo(-14 * bScale, -18 * bScale);
      ctx.closePath();
      ctx.fill();
      ...
      // Glowing Eyes
      ctx.fillStyle = isBoss ? '#ef4444' : '#fde047';
      ctx.fillRect(-8 * bScale, -22 * bScale, 5 * bScale, 5 * bScale);
      ctx.fillRect(3 * bScale, -22 * bScale, 5 * bScale, 5 * bScale);

      // Weapon (Club / Bow / Axe)
      ctx.fillStyle = '#78350f';
      ctx.fillRect(-22 * bScale, -8 * bScale, 8 * bScale, 28 * bScale);
      ctx.fillStyle = '#94a3b8';
      ctx.fillRect(-24 * bScale, -16 * bScale, 12 * bScale, 10 * bScale);
    } else {
      ctx.fillRect(-18, -15, 36, 42);
      ctx.fillStyle = '#fbbf24';
      ctx.fillRect(-8, -20, 6, 6);
      ctx.fillRect(2, -20, 6, 6);
      ctx.fillStyle = '#78716c';
      ctx.fillRect(16, -12, 8, 30);
    }

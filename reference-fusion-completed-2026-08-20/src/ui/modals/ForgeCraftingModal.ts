...
export const ForgeCraftingModal: ModalInstance = {
  ...
  render: () => {
    ...
    el.style.cssText = 'max-width:540px; padding:var(--space-16); background:radial-gradient(ellipse at 50% 15%, #1c1917 0%, #0c0a09 100%); border:2px solid #ef4444; border-radius:var(--radius-06); box-shadow:var(--shadow-modal);';
    ...
    const refresh = () => {
      ...
      el.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:var(--space-12); border-bottom:1.5px solid #78350f; padding-bottom:var(--space-10); flex-wrap:wrap; gap:var(--space-08);">
          <div style="display:flex; align-items:center; gap:var(--space-10);">
            <div style="width:42px; height:42px; border-radius:50%; background:rgba(0,0,0,0.7); border:2px solid #ef4444; display:flex; align-items:center; justify-content:center; box-shadow:var(--glow-danger); flex-shrink:0;">
            ...
            <div>
              ...
              <h3 style="font-family:var(--font-display); font-size:16px; color:#fef08a; margin:var(--space-01) 0 0 0;">
          ...
          <div style="display:flex; gap:var(--space-04);">
            ${unlockedSmiths.map((b) => `
              <button class="btn-switch-smith" data-smith-id="${b.id}" style="padding:var(--space-04) var(--space-08); font-size:10px; font-weight:bold; font-family:var(--font-display); background:${b.id === activeSmithId ? '#ef4444' : 'rgba(0,0,0,0.5)'}; border:1px solid #78350f; border-radius:var(--radius-03); color:#ffffff; cursor:pointer;">
                ${t(b.nameKey)}
              </button>
            `).join('')}
          </div>
        ...
        <div style="display:flex; gap:var(--space-06); margin-bottom:var(--space-12);">
          ${(['weapon', 'armor', 'accessory'] as EquipmentSlot[]).map((cat) => `
            <button class="btn-category-tab" data-category="${cat}" style="flex:1; padding:var(--space-06); font-size:11px; font-weight:bold; font-family:var(--font-display); background:${currentCategory === cat ? 'linear-gradient(135deg, #d97706, #b45309)' : 'rgba(28,25,23,0.8)'}; border:1px solid ${currentCategory === cat ? '#f59e0b' : '#78350f'}; border-radius:var(--radius-04); color:#ffffff; cursor:pointer;">
              ${t(`equipment.filter.${cat}`)}
            </button>
          `).join('')}
        </div>

        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:var(--space-10); margin-bottom:var(--space-14);">
          <div style="display:flex; flex-direction:column; gap:var(--space-06); max-height:220px; overflow-y:auto; padding-right:var(--space-04);">
            ${recipes.map((recipe) => {
              ...
              return `
                <div class="recipe-card-tile" data-recipe-id="${recipe.id}" style="background:${isSelected ? 'rgba(239,68,68,0.2)' : 'rgba(12,10,9,0.7)'}; border:1.5px solid ${isSelected ? '#ef4444' : '#78350f'}; border-radius:var(--radius-04); padding:var(--space-06) var(--space-08); cursor:pointer; transition:all 0.15s ease;">
            ...
            <div style="background:rgba(28,25,23,0.9); border:1.5px solid #b45309; border-radius:var(--radius-04); padding:var(--space-10); display:flex; flex-direction:column; justify-content:space-between;">
              <div>
                <div style="display:flex; align-items:center; gap:var(--space-08); margin-bottom:var(--space-06);">
                  <div style="width:34px; height:34px; border-radius:var(--radius-04); background:#0c0a09; border:1px solid #f59e0b; display:flex; align-items:center; justify-content:center;">${template.iconSvg}</div>
                  <div>
                ...
                <div style="font-size:10px; color:#cbd5e1; margin-bottom:var(--space-08); line-height:1.3;">
                ...
                <div style="background:rgba(0,0,0,0.5); padding:var(--space-06); border-radius:var(--radius-03); font-size:10px; margin-bottom:var(--space-08);">
                  <div style="color:#f59e0b; font-weight:bold; margin-bottom:var(--space-02); font-size:9px;">${t('forge.crafting_costs')}</div>
              ...
              <button id="btn-craft-action" ${!canCraft.canCraft ? 'disabled' : ''} style="width:100%; padding:var(--space-08); background:${canCraft.canCraft ? 'linear-gradient(135deg, #ef4444, #b91c1c)' : '#292524'}; border:1px solid ${canCraft.canCraft ? '#f87171' : '#451a03'}; border-radius:var(--radius-04); color:${canCraft.canCraft ? '#ffffff' : '#78716c'}; font-family:var(--font-display); font-weight:900; font-size:12px; letter-spacing:1px; cursor:${canCraft.canCraft ? 'pointer' : 'not-allowed'}; box-shadow:${canCraft.canCraft ? 'var(--glow-danger)' : 'none'};">
        ...
        <button id="btn-close-forge" style="width:100%; padding:var(--space-08); background:#1c1917; border:1px solid #78350f; border-radius:var(--radius-04); color:#cbd5e1; font-family:var(--font-display); font-size:12px; cursor:pointer;">
          ${t('forge.exit')}
        </button>
      `;

...
export type CharacterAnimationState = 'idle' | 'run' | 'attack' | 'crit' | 'hurt' | 'victory' | 'death';
...
export class PixelSpriteRenderer {
  ...
  public static getSwordsmanSprite(state: CharacterAnimationState = 'idle', rankColor: string = '#d97706'): string {
    ...
    const bodyClass = isDeath ? 'animate-defeat-fall' : isHurt ? 'animate-hurt' : isAttack ? 'animate-slash' : isCrit ? 'animate-crit-cleave' : isVictory ? 'animate-victory-cheer' : isRun ? 'animate-run-cycle' : 'animate-pixel-idle';

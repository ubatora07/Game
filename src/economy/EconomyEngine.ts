...
export class EconomyEngine {
  public static calculateMetrics(state: GameStateData, now: number = Date.now()): EconomyMetrics {
    ...
    const passivePowerPerSec = rawBuildingsPowerPerSec * totalGlobalPowerMult;
    const passiveGoldPerSec = rawBuildingsGoldPerSec * totalGlobalGoldMult;

    // 8. Decoupled Active Click / Manual Training Economy
    const comboMult = state.combo?.multiplier || 1.0;
    const clickPower = Math.max(1, Math.floor(
    ...
    const clickGold = Math.max(1, Math.floor(

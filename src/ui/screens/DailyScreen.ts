  ...
  private render(): void {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      daysHtml += `
        ...
        ">
          <div style="font-size: 10px; color: var(--text-muted); margin-bottom: 4px;">Day ${i}</div>
    ...
    const questsHtml = state.dailyQuests.map(q => {
      ...
      return `
        <div style="background: var(--bg-surface-raised); border: 1px solid ${q.claimed ? 'var(--border-subtle)' : isComplete ? 'var(--color-emerald)' : 'var(--border-subtle)'}; border-radius: var(--radius-md); padding: 12px; margin-bottom: 8px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            ...
            <button class="claim-quest-btn" data-id="${q.id}" ${(!isComplete || q.claimed) ? 'disabled' : ''} style="background: ${q.claimed ? 'transparent' : 'var(--color-emerald)'}; color: ${q.claimed ? 'var(--text-muted)' : '#000'}; padding: 6px 12px; border-radius: var(--radius-full); font-weight: bold;">
    ...
    this.el.innerHTML = `
      <h1 style="margin-bottom: 24px; color: var(--text-cyan); text-align: center;">📅 ${t('nav.dailies')}</h1>
      ...
      <div style="background: var(--bg-card); padding: 16px; border-radius: var(--radius-lg); margin-bottom: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          ...
          <button id="claimLoginBtn" ${state.loginRewardClaimed ? 'disabled' : ''} style="background: ${state.loginRewardClaimed ? 'var(--bg-core)' : 'var(--color-cyan)'}; color: ${state.loginRewardClaimed ? 'var(--text-muted)' : '#000'}; padding: 6px 16px; border-radius: var(--radius-full); font-weight: bold;">

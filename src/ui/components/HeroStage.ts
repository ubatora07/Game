...
export class HeroStage {
  ...
  private render(): void {
    this.el.innerHTML = `
      ...
      <!-- Power & Rate Header -->
      <div style="text-align:center; margin-bottom:6px; z-index:5;">
      ...
      <div id="heroAvatarContainer" style="position:relative; width:210px; height:210px; display:flex; align-items:center; justify-content:center; cursor:pointer; margin:4px 0; transition:transform 0.1s ease;">
        <!-- Aura Circle SVG -->
        ...
        <div id="heroGraphic" style="width:150px; height:150px; border-radius:50%; background:radial-gradient(circle, #1e293b 0%, #0f172a 100%); border:3px solid #94a3b8; display:flex; align-items:center; justify-content:center; box-shadow:0 0 25px rgba(148,163,184,0.4); animation:heroFloat 3.5s ease-in-out infinite; z-index:3; overflow:hidden;">
      ...
      <div style="width:100%; max-width:320px; margin-bottom:12px; z-index:5;">
        ...
        <div style="width:100%; height:10px; background:rgba(30,41,59,0.8); border-radius:var(--radius-full); overflow:hidden; border:1px solid var(--border-subtle); position:relative;">
          <div id="stageProgressBar" style="width:0%; height:100%; background:linear-gradient(90deg, #f59e0b, #fde047); border-radius:var(--radius-full); transition:width 0.2s ease; box-shadow:0 0 10px rgba(245,158,11,0.8);"></div>
        </div>
      </div>
      ...
      <button id="trainActionBtn" style="
        ...
        letter-spacing:2px;
        font-weight:900;
        box-shadow:0 6px 20px rgba(245,158,11,0.4), inset 0 2px 4px rgba(255,255,255,0.4);
        cursor:pointer;
        display:flex;
        align-items:center;
        justify-content:center;
      ...
      <button id="ascendQuickBtn" style="display:none; margin-top:10px; width:100%; max-width:320px; height:46px; background:linear-gradient(135deg, #7c3aed 0%, #a855f7 100%); border:2px solid #e9d5ff; border-radius:var(--radius-md); color:#ffffff; font-weight:bold; font-size:14px; animation:buttonReadyGlow 1.5s infinite; align-items:center; justify-content:center; gap:8px; cursor:pointer; z-index:5;">
        <span>🌟</span>
  ...
  private update(): void {
    ...
    if (heroGraphic) {
      ...
      heroGraphic.style.boxShadow = `0 0 30px ${rank.glowColor}`;
    }

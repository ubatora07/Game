...
export const ReincarnateModal: ModalInstance = {
  ...
  render: () => {
    ...
    el.innerHTML = `
      <div style="font-size:48px; margin-bottom:var(--space-06);">🔄</div>
      <h2 style="font-family:var(--font-display); font-size:22px; color:#f43f5e; margin-bottom:var(--space-08);">
        ${t('modal.reincarnate.title')}
      </h2>

      <p style="color:var(--text-muted); font-size:12px; margin-bottom:var(--space-14); line-height:1.5;">
        ${t('modal.reincarnate.warning')}
      </p>

      <div style="background:rgba(30,41,59,0.7); border:1px solid #f43f5e; border-radius:var(--radius-md); padding:var(--space-16); margin-bottom:var(--space-10); box-shadow:var(--glow-gold);">
      ...
      ${requirements.canRebirth ? '' : `
        <div style="font-size:11px; color:var(--text-muted); margin-bottom:var(--space-14);">
      ...
      <div style="display:flex; gap:var(--space-10);">
        ...
        <button id="confirmReincarnateBtn" ${requirements.canRebirth ? '' : 'disabled aria-disabled="true"'} style="flex:1; height:46px; background:${requirements.canRebirth ? 'linear-gradient(135deg, #e11d48, #be123c)' : 'rgba(51,65,85,0.5)'}; border:1px solid ${requirements.canRebirth ? '#f43f5e' : 'var(--border-subtle)'}; border-radius:var(--radius-md); color:${requirements.canRebirth ? '#ffffff' : '#64748b'}; font-weight:bold; cursor:${requirements.canRebirth ? 'pointer' : 'not-allowed'}; box-shadow:${requirements.canRebirth ? 'var(--glow-danger)' : 'none'};">
          ${t('btn.confirm')}
        </button>
      </div>
    `;

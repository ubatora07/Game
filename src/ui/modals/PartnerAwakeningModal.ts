...
export const PartnerAwakeningModal: ModalInstance = {
  ...
  render: (data?: { defaultClass?: CharacterClassId }) => {
    ...
    el.innerHTML = `
      <div style="font-size:48px; margin-bottom:8px; animation:heroFloat 2s ease-in-out infinite;">
        ✨👥✨
      </div>
      <h2 style="font-family:var(--font-display); font-size:24px; color:#c084fc; letter-spacing:1px; text-shadow:0 0 15px rgba(192,132,252,0.6); margin-bottom:6px;">
        ${t('modal.partner_awakening.title')}
      </h2>
      <p style="color:var(--text-muted); font-size:12px; margin-bottom:14px; line-height:1.4;">
        ${t('modal.partner_awakening.desc')}
      </p>

      <!-- Name Input -->
      <div style="margin-bottom:14px; text-align:left;">
        <label style="font-size:11px; color:#cbd5e1; font-weight:bold; display:block; margin-bottom:4px;">
          ${t('modal.partner_awakening.name_label')}
        </label>
        <input id="partnerNameInput" type="text" value="${partnerName}" style="width:100%; height:36px; background:rgba(15,23,42,0.8); border:1px solid #c084fc; border-radius:6px; padding:0 10px; color:#ffffff; font-size:13px; outline:none;" />
      </div>

      <!-- Class Selection Grid -->
      <div style="margin-bottom:16px; text-align:left;">
        <label style="font-size:11px; color:#cbd5e1; font-weight:bold; display:block; margin-bottom:6px;">
          ${t('modal.partner_awakening.class_label')}
        </label>
        <div class="partner-class-grid" style="display:grid; grid-template-columns:repeat(2, 1fr); gap:8px;">
          ${classes
            .map((c) => {
              const isSelected = c.id === selectedClass;
              return `
                <div class="partner-class-card ${isSelected ? 'selected' : ''}" data-class-id="${c.id}" style="border:2px solid ${isSelected ? c.themeColor : 'rgba(255,255,255,0.1)'}; background:rgba(15,23,42,0.9); border-radius:6px; padding:8px; cursor:pointer; display:flex; align-items:center; gap:8px;">
      ...
      <button id="btn-awaken-partner" style="width:100%; height:46px; background:linear-gradient(135deg, #9333ea, #c084fc); border:1px solid #f3e8ff; border-radius:8px; color:#ffffff; font-weight:bold; font-size:16px; cursor:pointer; box-shadow:0 0 15px rgba(168,85,247,0.5);">

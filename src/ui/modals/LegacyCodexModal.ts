...
export const LegacyCodexModal: ModalInstance = {
  ...
  render: () => {
    ...
    const refresh = () => {
      ...
      el.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1.5px solid #7c3aed; padding-bottom:8px; gap:8px;">
          <div>
            ...
            <h3 style="font-family:var(--font-display); font-size:16px; color:#fef08a; margin:1px 0 0 0;">${t('legacy.codex.title')}</h3>
          </div>
          <button id="btn-close-codex" style="background:#18181b; border:1px solid #7c3aed; border-radius:4px; color:#c084fc; padding:4px 8px; font-family:var(--font-display); font-size:11px; cursor:pointer;">✕ ${t('btn.close')}</button>
        </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:6px; margin-bottom:12px;">
          ${allEndings.map((endingDef) => {
            ...
            return `
              <button class="btn-ending-card" data-ending-id="${endingDef.id}" style="display:flex; align-items:center; gap:8px; padding:8px; border-radius:4px; border:1.5px solid ${active ? '#c084fc' : unlocked ? '#581c87' : '#27272a'}; background:${active ? 'rgba(168,85,247,0.25)' : 'rgba(0,0,0,0.6)'}; text-align:left; cursor:pointer;">
        ...
        <div style="background:rgba(24,24,27,0.9); border:1.5px solid #7c3aed; border-radius:4px; padding:12px; margin-bottom:12px;">
          ...
          <div style="font-size:9px; color:#94a3b8; margin-bottom:8px; background:rgba(0,0,0,0.4); padding:4px 6px; border-radius:3px;"><b>${t('legacy.codex.unlock_condition')}:</b> ${t(currentDef.requirementKey)}</div>
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; background:${isUnlocked ? 'rgba(88,28,135,0.4)' : 'rgba(39,39,42,0.4)'}; border:1px dashed ${isUnlocked ? '#c084fc' : '#52525b'}; padding:8px 10px; border-radius:4px; font-size:10px; color:${isUnlocked ? '#a7f3d0' : '#71717a'}; gap:8px;">
            <div>
            ...
            ${isUnlocked ? `
              <button id="btn-equip-boon" data-id="${currentDef.id}" style="background:${activeBoonId === currentDef.id ? 'linear-gradient(180deg, #10b981, #059669)' : 'linear-gradient(180deg, #8b5cf6, #6d28d9)'}; border:1px solid #c084fc; border-radius:4px; color:#fff; font-size:10px; font-weight:bold; padding:4px 10px; cursor:pointer;">

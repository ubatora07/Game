...
export const MarketModal: ModalInstance = {
  ...
  render: () => {
    ...
    el.style.cssText = 'max-width:580px; padding:var(--space-16); background:radial-gradient(ellipse at 50% 15%, #1c1917 0%, #0c0a09 100%); border:2px solid #f59e0b; border-radius:var(--radius-06); box-shadow:var(--shadow-modal);';
    ...
    const refresh = () => {
      ...
      el.innerHTML = `
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:var(--space-12); border-bottom:1.5px solid #78350f; padding-bottom:var(--space-08); flex-wrap:wrap; gap:var(--space-08);">
          ...
          <!-- Currency Pills -->
          <div style="display:flex; gap:var(--space-06); font-size:11px;">
            <span style="background:rgba(0,0,0,0.5); padding:var(--space-03) var(--space-08); border-radius:var(--radius-04); border:1px solid #eab308; color:#fef08a;">
              🪙 <b>${store.get().gold}</b>
            </span>
            <span style="background:rgba(0,0,0,0.5); padding:var(--space-03) var(--space-08); border-radius:var(--radius-04); border:1px solid ${karmaScore < 0 ? '#f43f5e' : '#34d399'}; color:${karmaScore < 0 ? '#f43f5e' : '#34d399'}; font-weight:bold;">
        ...
        <div style="display:flex; gap:var(--space-04); margin-bottom:var(--space-10); flex-wrap:wrap;">
          ${(['all', 'materials', 'settlement', 'equipment', 'mercenaries', 'titles'] as MarketCategory[])
            .map(
              (cat) => `
            <button class="btn-market-cat" data-category="${cat}" style="padding:var(--space-04) var(--space-08); font-size:10px; font-weight:bold; font-family:var(--font-display); background:${currentCategory === cat ? 'linear-gradient(135deg, #d97706, #b45309)' : 'rgba(0,0,0,0.5)'}; border:1px solid ${currentCategory === cat ? '#f59e0b' : '#78350f'}; border-radius:var(--radius-03); color:#ffffff; cursor:pointer;">
          ...
          ${
            isBlackMarketAvailable
              ? `
            <button class="btn-market-cat" data-category="black_market" style="padding:var(--space-04) var(--space-08); font-size:10px; font-weight:bold; font-family:var(--font-display); background:${currentCategory === 'black_market' ? 'linear-gradient(135deg, #7c3aed, #4c1d95)' : 'rgba(46,16,101,0.5)'}; border:1px solid #a855f7; border-radius:var(--radius-03); color:#e9d5ff; cursor:pointer; box-shadow:var(--glow-purple);">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1.1fr; gap:var(--space-10); margin-bottom:var(--space-12);">
            ...
            ${
              offers.length > 0
                ...
                `;
                    ...
                    .join('')
                : `<div style="color:#94a3b8; font-size:11px; text-align:center; padding:var(--space-20);">${t('market.no_items')}</div>`
            }
          </div>
          ...
          ${
            selectedOffer
              ? `
            <div style="background:${selectedOffer.isBlackMarket ? 'rgba(30,12,50,0.9)' : 'rgba(28,25,23,0.9)'}; border:1.5px solid ${selectedOffer.isBlackMarket ? '#a855f7' : '#b45309'}; border-radius:var(--radius-04); padding:var(--space-10); display:flex; flex-direction:column; justify-content:space-between;">
              <div>
                <div style="display:flex; align-items:center; gap:var(--space-08); margin-bottom:var(--space-06);">
                  <div style="width:34px; height:34px; border-radius:var(--radius-04); background:#0c0a09; border:1px solid ${selectedOffer.isBlackMarket ? '#a855f7' : '#f59e0b'}; display:flex; align-items:center; justify-content:center;">
                ...
                ${
                  selectedOffer.tradeOffDesc
                    ? `
                  <div style="background:rgba(244,63,94,0.15); border:1px solid #f43f5e; border-radius:var(--radius-03); padding:var(--space-06); font-size:9px; color:#fca5a5; margin-bottom:var(--space-08); font-weight:bold;">
              ...
              <button id="btn-buy-offer" ${!canBuy.canBuy ? 'disabled' : ''} style="width:100%; padding:var(--space-08); background:${canBuy.canBuy ? (selectedOffer.isBlackMarket ? 'linear-gradient(135deg, #7c3aed, #4c1d95)' : 'linear-gradient(135deg, #10b981, #059669)') : '#292524'}; border:1px solid ${canBuy.canBuy ? (selectedOffer.isBlackMarket ? '#a855f7' : '#34d399') : '#451a03'}; border-radius:var(--radius-04); color:${canBuy.canBuy ? '#ffffff' : '#78716c'}; font-family:var(--font-display); font-weight:bold; font-size:12px; cursor:${canBuy.canBuy ? 'pointer' : 'not-allowed'}; box-shadow:${canBuy.canBuy ? 'var(--glow-success)' : 'none'};">
        ...
        <div style="display:flex; gap:var(--space-06);">
          <button id="btn-refresh-market" style="padding:var(--space-06) var(--space-12); background:#451a03; border:1px solid #78350f; border-radius:var(--radius-04); color:#fde047; font-family:var(--font-display); font-size:11px; font-weight:bold; cursor:pointer;">
            🔄 ${t('market.refresh_stock')}
          </button>
          <button id="btn-close-market" style="flex:1; padding:var(--space-06); background:#1c1917; border:1px solid #78350f; border-radius:var(--radius-04); color:#cbd5e1; font-family:var(--font-display); font-size:11px; cursor:pointer;">
            ${t('market.exit')}
          </button>
        </div>
      `;

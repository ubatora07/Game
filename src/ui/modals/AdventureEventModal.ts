    ...
    el.innerHTML = `
      ...
      <div class="adventure-choices-list" style="display:flex; flex-direction:column; gap:var(--space-08);">
        ${evt.choices
          .map(
            (c) => `
          <button class="btn-adventure-choice" data-choice-id="${c.id}" style="width:100%; padding:var(--space-10) var(--space-14); background:linear-gradient(180deg, #292524 0%, #1c1917 100%); border:1px solid #b45309; border-radius:var(--radius-04); color:#fef08a; font-size:12px; font-weight:bold; cursor:pointer; text-align:left; display:flex; justify-content:space-between; align-items:center; transition:all 0.15s ease; font-family:var(--font-display); box-shadow:var(--shadow-sm);">
